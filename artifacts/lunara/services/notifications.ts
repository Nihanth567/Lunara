...
async function scheduleReminderForDate(date: Date): Promise<void> {
  await Notifications.scheduleNotificationAsync({
    ...
    content: {
      title: "Tonight's ritual is waiting 🌙",
      body: 'Take a moment to share with your partner before the day ends.',
      data: {
        screen: 'tonight',
...
/**
 * Send an immediate local notification confirming the nudge.
 * (Until a real backend exists the partner won't literally receive it,
 *  but the sender sees confirmation and the UX is wired correctly.)
 */
export async function sendNudgeNotification(partnerName: string): Promise<void> {
  if (Platform.OS === 'web') return;
  await Notifications.scheduleNotificationAsync({
    content: {
      title: `Nudge sent to ${partnerName} ✨`,
      body: "They'll know you're thinking of them.",
      data: { screen: 'tonight' },
      sound: false,
    },
