...
function AlreadyRevealedState({ onView }: { onView: () => void }) {
  ...
  return (
    <Animated.View style={[styles.revealedCard, animStyle]}>
      ...
      <Text style={styles.revealedBody}>Rest well — new cards will be waiting for you both tomorrow night.</Text>
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* Streak — prominent, calm, always visible once paired */}
          {couple && (
            <StreakSpotlight streak={streak} longestStreak={couple.longestStreak ?? 0} />
          )}
              ...
              {!allFilled && (
                <View style={styles.progressHint}>
                  <Text style={styles.progressText}>
                    {[grateful, cute, grow].filter((v) => v.trim().length > 0).length} of 3 shared so far
                  </Text>
                </View>
              )}
            </Animated.View>
          ...
          {isSubmitted && (
             <Animated.View style={styles.partnerBadge}>
              ...
              <Text style={styles.partnerBadgeText}>
                {partnerSubmitted
                  ? `${partnerName} has shared their heart tonight`
                  : `Still waiting quietly for ${partnerName}`}
              </Text>
            </Animated.View>
          )}
        </ScrollView>
